...
def make_preprocessor(processes: List[str], os_rate: float, us_rate: float, cur_ratio: float, n_components: int) -> List[tuple]:
    ...
    if processes is None:
        return []
    sampling_strategy = 3/7
    preps = []
    if 'ROS' in processes:
        preps.append(('ROS', RandomOverSampler(sampling_strategy=sampling_strategy, random_state=0)))
        cur_ratio = cur_ratio * os_rate
    if 'SMOTE' in processes:
        preps.append(('SMOTE', SMOTE(sampling_strategy=sampling_strategy, random_state=0)))
        cur_ratio = cur_ratio * os_rate
    if 'RUS' in processes:
        preps.append(('RUS', RandomUnderSampler(sampling_strategy=sampling_strategy, random_state=0)))
