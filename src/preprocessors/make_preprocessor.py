...
def make_preprocessor(processes: List[str], os_rate: float, us_rate: float, cur_ratio: float, n_components: int) -> List[tuple]:
    ...
    if 'ROS' in processes:
        ...
        preps.append(('ROS', RandomOverSampler(sampling_strategy=cur_ratio, random_state=0)))
        print(f"ratio after ROS is {cur_ratio}")
    if 'SMOTE' in processes:
        #cur_ratio = cur_ratio * os_rate
        preps.append(('SMOTE', SMOTE(sampling_strategy=cur_ratio, random_state=0)))
        print(f"ratio after SMOTE is {cur_ratio}")
    if 'RUS' in processes:
        #cur_ratio = cur_ratio * os_rate
        preps.append(('RUS', RandomUnderSampler(sampling_strategy=cur_ratio, random_state=0)))
        print(f"ratio after RUS is {cur_ratio}")
